...
static void fast_loop_core_pd0(ModeDecisionCandidateBuffer* cand_bf, PictureControlSet* pcs, ModeDecisionContext* ctx,
    ...
    /* [SVT_HDR_MODE] mainline uses the VAR fast-cost unconditionally; the svt-av1-hdr
       fork gates on mds0_dist_type and adds a spatial-distortion path carrying
       ac_bias/tx_bias. (The rc-era VLPD0 inter branch was dead code and is gone in
       v4.2.0-final.) */
#if SVT_HDR_MODE
...
static INLINE uint32_t compute_lpd0_cost_allintra(PictureControlSet* pcs, ModeDecisionContext* ctx) {
    ...
    SvtVarType* sb_var = pcs->ppcs->variance[ctx->sb_index];
